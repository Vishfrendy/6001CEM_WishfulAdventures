<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if ($action === "create") {
			...
			createRestaurants($name, $description, $rating, $min_price, $max_price, $image_path);
			...
			$max_price = isset($_POST["max_price"]) ? $_POST["max_price"] : '';
			$image_path = isset($_POST["image_path"]) ? $_POST["image_path"] : '';
			updateRestaurants($name, $description, $rating, $min_price, $max_price, $image_path);
		} elseif ($action === "delete") {
			$name = isset($_POST["name"]) ? $_POST["name"] : '';
			deleteRestaurants($name);
		}
	}

	// Create a new restaurants
	function createRestaurants($name, $description, $rating, $min_price, $max_price, $image_path) {
		global $conn;
		$sql = "INSERT INTO restaurants (name, description, rating, min_price, max_price, image_path) VALUES ('$name', '$description', '$rating', '$min_price', '$max_price', '$image_path')";
		$conn->query($sql);
	}

	// Read restaurants details by name
	function readRestaurants($name) {
		global $conn;
	...
	// Update restaurants details by name
	function updateRestaurants($name, $description, $rating, $min_price, $max_price, $image_path) {
		global $conn;
		$sql = "UPDATE restaurants SET description = '$description', rating = '$rating', min_price = '$min_price', max_price = '$max_price', image_path = '$image_path' WHERE name = '$name'";
		$conn->query($sql);
	}

	// Delete restaurants by name
	function deleteRestaurants($name) {
		global $conn;
...
<html>
	<head>
		...
		<style>
			...
			/* Additional styles for the restaurants list table */
			table {
				width: 100%;
	...
	<body>
					...
					<form class="form-container" style="position: relative; left: 275px; margin-top: 27px;">
						...
						<table>
							...
							<?php
								...
								// Retrieve limited restaurants from the database
								$sql = "SELECT * FROM restaurants LIMIT $offset, $recordsPerPage";
								$result = $conn->query($sql);
									
								if ($result->num_rows > 0) {
									// Within the while loop that displays restaurant details in the table
							...
							<?php
								...
								if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["name"])) {
									...
									// For example, you can call the deleteRestaurants() function from the main PHP file.
									deleteRestaurants($name);
									// Redirect back to the main page (Admin_Restaurants.php) after deletion.
									header("Location: Admin_Restaurants.php");
									exit();
								}
							?>
						...
						<div class="pagination">
							<?php
								...
								$totalCountSql = "SELECT COUNT(*) as total FROM restaurants";
		...
		<!---------- Script ---------->
		<script>
			// JavaScript function to display the delete confirmation message with the restaurant name
			function confirmDelete(restaurantName) {
				if (confirm('Are you sure you want to delete the restaurant "' + restaurantName + '"?')) {
					// Redirect to the delete_restaurant.php file with the restaurant name as a parameter
					location.href = 'delete_restaurant.php?name=' + restaurantName;
				}
			}
		</script>
		...
		<script>
			...
			function deleteRestaurant() {
				// Get the restaurant name from the modal
				const restaurantName = document.getElementById('restaurantName').innerText;
			  
				// Redirect to the Admin_Restaurants.php file with the restaurant name as a parameter
				location.href = 'Admin_Restaurants.php?name=' + restaurantName;
			}
		</script>
		
		<script>
			// Function to open the edit modal and pre-fill fields with restaurant details
			function openEditModal(name, description, rating, min_price, max_price, image_path) {
				const modal = document.getElementById('editModal');
				modal.style.display = 'block';
			  
				// Pre-fill form fields with restaurant details
