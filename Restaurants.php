	...
	<body>
		...
		<div class="container">
			...
			<div class="links-container">
				...
				<a href="#"><img src="Restaurant.png" alt="Restaurant" style="position: relative; height: 20px; width: 20px; margin-right: 8px; top: 3px;">Restaurants</a>
			...
			<?php
				...
				while ($row = mysqli_fetch_assoc($result)) {
					...
					echo '</div>';
					echo '<p class="price">RM' . $row['min_price'] . ' - RM' . $row['max_price'] . '</p>';
					echo '<button class="view-price-button" data-restaurant-id="' . $row['restaurant_id'] . '">View Price</button>';
					echo '</div>';
				}
		...
		<script>
			...
			// JavaScript function to handle the "View Price" button click
			function handleViewPriceClick(button) {
				const restaurantId = button.getAttribute("data-restaurant-id");
				// You can implement the logic to display the price or perform any other action here.
				alert("View Price button clicked for restaurant ID: " + restaurantId);
			}

			// Event listener to handle "View Price" button click
			document.addEventListener("DOMContentLoaded", function () {
				const viewPriceButtons = document.querySelectorAll(".view-price-button");
				viewPriceButtons.forEach((button) => {
					button.addEventListener("click", function () {
						handleViewPriceClick(button);
					});
				});
			});
		</script>
